...
/* =========================
   개별 종목 Row (비즈니스 로직 집중)
========================= */
function HoldingStockRow({ holdingStock, initialTargets }: HoldingStockRowProps) {
  const [stockData, setStockData] = useState<{ currentPrice: number; rate: number } | null>(null);
  ...
  // 1. 초기 데이터 및 시세 로드
  useEffect(() => {
    let isFirstLoad = true; // 최초 1회 세팅을 위한 플래그
    ...
    async function getStockData() {
      ...
      if (data) {
        ...
        setStockData({ currentPrice, rate });

        // 2. 초기값 세팅 (최초 로드 시 혹은 수정 중이 아닐 때만)
        if (isFirstLoad) {
          // 상한가 세팅
  ...
  }, [holdingStock.stock.stockCode, initialTargets]); // initialTargets가 변경될 때만 재실행
...
export default function HistoryAsset({ portfolioId }: Props) {
  ...
  useEffect(() => {
    if (portfolioId) {
      ...
      fetchPriceTargets().then(data => {
        if (data) setExistingTargets(data); // 주신 JSON 형태의 리스트가 여기 담김
      });
    }
  ...
  useEffect(() => {
    const calcTotalMetrics = async () => {
      ...
      const priceResults = await Promise.all(holdings.map((h) => fetchStockCurrentPrice(h.stock.stockCode)));
      let totalPurchase = 0;
      let totalValue = 0;
      ...
      const returnRate = totalPurchase > 0 ? ((totalValue - totalPurchase) / totalPurchase) * 100 : 0;
      setSummary({ totalPurchaseAmount: totalPurchase, totalCurrentValue: totalValue, totalReturnRate: returnRate });
    };
    calcTotalMetrics();
    const id = setInterval(calcTotalMetrics, 3000);
    return () => clearInterval(id);
  }, [holdings]);
